DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_consulta_novedades_manejo`(IN _idobra INT, IN _idconcepto1  INT,IN _idconcepto2  INT,IN _idconcepto3  INT, IN  _fechaini date ,IN  _fechafin date)
BEGIN
SELECT
	r.id,
    r.rem_porc_sum,
	...
	if(isnull(n2.rmnv_valor_subtotal), d.dtrm_cantdespachada * d.dtrm_precio,rmnv_valor_subtotal) as  subtotalrem,
...
FROM
  ...
  /*  n1.obranov*/
	...
	and r.remi_estado ='A'
    and o.obra_estado ='A'
	and r.id in (select id_remission from settlementremissions );
